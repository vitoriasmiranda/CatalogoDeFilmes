...
Um catálogo de filmes construído com React, consumindo dados em tempo real da API pública do [The Movie Database (TMDB)](https://www.themoviedb.org/). O projeto permite navegar por filmes populares, buscar títulos específicos e visualizar detalhes completos de cada filme, com suporte a tema claro e escuro.
...
- **[React](https://react.dev/)** — biblioteca para construção da interface, utilizando componentes funcionais e Hooks (`useState`, `useEffect`)
- **[Vite](https://vitejs.dev/)** — ferramenta de build e servidor de desenvolvimento, responsável pelo ambiente rápido de desenvolvimento e pela leitura das variáveis de ambiente
- **[React Router DOM](https://reactrouter.com/)** — biblioteca de roteamento, utilizada para navegação entre páginas (`BrowserRouter`, `Routes`, `Route`, `Link`) e captura de parâmetros dinâmicos na URL (`useParams`)
...
- **[TMDB API](https://developer.themoviedb.org/)** — fonte dos dados de filmes, consumida via `fetch` com tratamento assíncrono (`async`/`await`)
